"""
Solid reagents tab implementation.

This module handles the solid reagent entry interface, extracted from the
...
class SolidReagentsTab:
    """
    Tab for solid reagent entry and management.
    
    This class handles all UI logic specific to solid reagents,
    delegating business logic to the ReagentService.
    """
    
    def __init__(self, reagent_service: ReagentService):
        """
        Initialize solid reagents tab.
    ...
    def _create_tab(self) -> widgets.Widget:
        """Create the solid reagents tab interface."""
        
        # Create header
        header = SectionHeader.create(
            "🧱 Solid Reagents",
            description="Add solid reagents (powders, crystals, etc.)"
        )
        ...
        self.widget = widgets.VBox([
            header,
            widgets.HTML("<p>Add solid reagents to your experiment. These are typically powders, crystals, or other solid compounds.</p>"),
    ...
    def _handle_save_reagent(self, form_data: Dict[str, Any], editing_reagent: Optional[ReagentModel]) -> bool:
        ...
        try:
            ...
            print(f"Error in solid reagents tab: {e}")
            return False
    ...
    def start_editing_reagent(self, reagent: ReagentModel):
        ...
        if reagent.reagent_type != "solid":
            raise ValueError("Cannot edit non-solid reagent in solid reagents tab")
...
def create_solid_reagents_tab(reagent_service: ReagentService) -> SolidReagentsTab:
    """
    Factory function to create a solid reagents tab.
    
    Args:
        reagent_service: Service for reagent operations
        
    Returns:
        SolidReagentsTab instance
    """
    return SolidReagentsTab(reagent_service)


__all__ = ['SolidReagentsTab', 'create_solid_reagents_tab']