...
Updated to use the new modular architecture with centralized imports.
"""
...
class ReagentDataAdapter:
    ...
    def to_experiment_model(self) -> ExperimentModel:
        """
        Convert current data to an ExperimentModel instance.
        ...
        return ExperimentModel.from_old_format(self._cached_data)
    
    def from_experiment_model(self, experiment: ExperimentModel) -> bool:
        """
        Update the experimental metadata from an ExperimentModel.
        ...
        """
        try:
            old_format_data = experiment.to_old_format()
            
            # Clear existing data
            chemistry_data = {"solid_reagents": [], "liquid_reagents": []}
            
            # Add solid reagents
            for reagent_data in old_format_data.get("solid reagents", []):
                new_reagent = self._convert_to_new_format(reagent_data)
                chemistry_data["solid_reagents"].append(new_reagent)
            
            # Add liquid reagents
            for reagent_data in old_format_data.get("liquid reagents", []):
                new_reagent = self._convert_to_new_format(reagent_data)
                chemistry_data["liquid_reagents"].append(new_reagent)
            
            # Add experiment parameters
            if "mass scale (in mg)" in old_format_data:
                chemistry_data["mass_scale"] = old_format_data["mass scale (in mg)"]
            if "concentration (in mM)" in old_format_data:
                chemistry_data["concentration"] = old_format_data["concentration (in mM)"]
            if "solvent" in old_format_data:
                chemistry_data["solvent"] = old_format_data["solvent"]
    ...
    def add_reagent_model(self, reagent: ReagentModel) -> bool:
        """
        Add a ReagentModel to the data.
        
        Args:
            reagent: ReagentModel instance to add
        ...
        return self.add_reagent(old_format_data, reagent.reagent_type)
    
    def update_reagent_model(self, old_reagent: ReagentModel, new_reagent: ReagentModel) -> bool:
        """
        Update a ReagentModel in the data.
        ...
        old_format_old = old_reagent.to_old_format()
        old_format_new = new_reagent.to_old_format()
        
        return self.update_reagent(old_format_old, old_format_new, new_reagent.reagent_type)