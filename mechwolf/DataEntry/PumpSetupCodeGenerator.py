...
class PumpCodeGenerator:
    ...
    def create_widgets(self):
        ...
        self.serial_port_text = widgets.Text(
            value="COM1", description="Serial Port:", layout=widgets.Layout(width="50%")
        )
        ...
        self.mcu_id_text = widgets.Text(
            value="", description="MCU ID:", layout=widgets.Layout(width="50%")
        )

        self.motor_id_text = widgets.Text(
            value="", description="Motor ID:", layout=widgets.Layout(width="50%")
        )
        ...
        self.unit_id_text = widgets.IntText(
            value=0, description="Unit ID:", layout=widgets.Layout(width="40%")
        )
        ...
        self.add_pump_button = widgets.Button(
            description="Add Pump", button_style="success", icon="plus"
        )
        ...
        self.generate_code_button = widgets.Button(
            description="Generate Code", button_style="primary", icon="code"
        )

        # Clear all button
        self.clear_all_button = widgets.Button(
            description="Clear All", button_style="danger", icon="trash"
        )
    ...
    def display_ui(self):
        ...
        ui_container = widgets.VBox(
            [
                ...
                # Pump list section
                widgets.VBox(
                    [widgets.HTML("<h3>Configured Pumps</h3>"), self.pump_list_output]
                ),
                ...
                widgets.VBox(
                    [
                        ...
                        widgets.HBox(
                            [self.generate_code_button, self.clear_all_button]
                        ),
                        self.code_output,
                    ]
                ),
    ...
    def add_pump(self, _):
        ...
        pump_config = {"type": pump_type, "serial_port": serial_port}
        ...
        with self.code_output:
            ...
            for pump_type in sorted(pump_types):
                if pump_type == "HarvardSyringePump":
                    ...
                    code.append(
                        "from mechwolf.components.contrib.vicipump import ViciPump"
                    )
