...
from mechwolf.DataEntry.ReagentUI.FormHandlers import (
    FinalDetailsFormHandler,
    ReagentFormHandler,
)
...
class ReagentInputForm:
    ...
    def setup_ui(self) -> None:
        ...
        self.main_container = widgets.VBox(
            [widgets.HTML("<h3>Reagent Entry Form</h3>"), self.tab_container]
        )
    ...
    def create_add_reagents_tab(self) -> widgets.Widget:
        ...
        return widgets.VBox(
            [widgets.HTML("<h4>Add New Reagents</h4>"), reagent_accordion],
    ...
    def create_current_reagents_tab(self) -> widgets.Widget:
        ...
        # Return the complete tab content
        return widgets.VBox(
            [widgets.HTML("<h4>Current Reagents</h4>"), self.reagent_list],
            layout=widgets.Layout(padding="10px"),
        )
    ...
    def create_final_details_tab(self) -> widgets.Widget:
        ...
        form = FinalDetailsFormHandler.create_final_details_form(
            self.data_manager.data, on_submit=self.process_final_details
        )

        # Return the complete tab content
        return widgets.VBox(
            [widgets.HTML("<h4>Final Details and Submission</h4>"), form],
            layout=widgets.Layout(padding="10px"),
        )
    ...
    def create_search_tab(self) -> widgets.Widget:
        ...
        search_button = widgets.Button(
            description="Search PubChem", button_style="info", icon="search"
        )
        ...
        def on_search_click(b):
            ...
            if not query:
                self.search_status.value = (
                    "<p style='color: orange;'>Please enter a search term</p>"
                )
                return

            self.search_status.value = (
                "<p style='color: blue;'>Searching PubChem...</p>"
            )
            ...
            self.search_status.value = (
                f"<p style='color: green;'>Found {len(results)} results</p>"
            )
        ...
        results_container = widgets.VBox(
            [widgets.HTML("<h4>Search Results</h4>"), self.search_results],
            layout=widgets.Layout(
                border="1px solid #ddd",
                padding="10px",
                margin="10px 0",
    ...
    def save_reagent(self, new_reagent, old_reagent=None, specified_type=None):
        ...
        try:
            ...
            missing_fields = [
                field for field in required_fields if field not in new_reagent
            ]
            ...
            if specified_type:
                ...
                reagent_type = (
                    "liquid" if "density (in g/mL)" in new_reagent else "solid"
                )
            ...
            reagent_copy["molecular weight (in g/mol)"] = float(
                reagent_copy["molecular weight (in g/mol)"]
            )
            reagent_copy["eq"] = float(reagent_copy["eq"])
            reagent_copy["syringe"] = int(reagent_copy["syringe"])
            if "density (in g/mL)" in reagent_copy:
                reagent_copy["density (in g/mL)"] = float(
                    reagent_copy["density (in g/mL)"]
                )
            ...
            if old_reagent:
                self.data_manager.update_reagent(
                    old_reagent, reagent_copy, reagent_type
                )
            else:
    ...
    def edit_reagent(self, reagent):
        ...
        form = ReagentFormHandler.create_reagent_form(
            reagent_type, reagent=reagent, on_save=self.save_reagent
        )
    ...
    def lookup_chemical(
        self, name, smiles_input, inchi_input, inchikey_input, mw_input, update_callback
    ):
    ...
    def import_from_pubchem(self, compound, reagent_type):
        ...
        form = ReagentFormHandler.create_reagent_form(
            reagent_type,
            reagent=new_reagent,
            on_save=lambda new, old=None: self.save_reagent(
                new, old, specified_type=reagent_type
            ),
        ...
        if reagent_type == "liquid" and has_density:
            success_message += (
                f" Density value ({compound['density']} g/mL) retrieved from PubChem."
            )
    ...
    def process_final_details(self, mass_scale, concentration, solvent, message_area):
        ...
        try:
            ...
            if not self.data_manager.has_limiting_reagent():
                raise ValueError(
                    "At least one reagent must have an equivalent (eq) value of 1.0"
                )
    ...
    def run(self) -> None:
        ...
        if (
            not self.data_manager.data["solid reagents"]
            and not self.data_manager.data["liquid reagents"]
        ):
            self.setup_ui()
        else:
            ...
            display(
                widgets.HBox(
                    [edit_btn, continue_btn], layout=widgets.Layout(margin="20px 0")
                )
            )
