...
class ProtocolAlgorithm:
    ...
    def _get_pump_vessel_mapping(self):
    ...
    def validate_inputs(self, configs):
        """Validate all inputs from GUI"""
        for config in configs:
            if not self.validate_flow_rate(str(config.flow_rate)) or config.flow_rate <= 0:
                raise ValueError(f"Invalid flow rate: {config.flow_rate}. Please enter a positive number followed by 'mL/min'.")
            if not self.validate_volume(str(config.volume)) or config.volume <= 0:
                raise ValueError(f"Invalid volume: {config.volume}. Please enter a positive number followed by 'mL'.")
            if not self.validate_time(str(config.delay)) or config.delay < 0:
                raise ValueError(f"Invalid delay time: {config.delay}. Please enter a non-negative number followed by 'seconds'.")
        return True

    def validate_flow_rate(self, flow_rate):
        return bool(re.match(r'^\d+(\.\d+)?(?:\s*(?:mL/min|ML/Min|ml/min|ML/min|ML/MIN))?$', flow_rate))

    def validate_volume(self, volume):
        return bool(re.match(r'^\d+(\.\d+)?(?:\s*(?:mL|ML|ml|Ml))?$', volume))

    def validate_time(self, time):
        return bool(re.match(r'^\d+(\.\d+)?(?:\s*(?:seconds|sec|s|S|SEC|SECONDS|SEc|Sec))?$', time))

    def validate_pump_volumes(self, configs):
    ...
    def create_protocol(self):
        try:
            ...
            if self.validate_inputs(gui.configs) and self.validate_pump_volumes(gui.configs):
                ...
                for config in gui.configs:
                    ...
                    active_time = timedelta(seconds=(config.volume / config.flow_rate * 60))
