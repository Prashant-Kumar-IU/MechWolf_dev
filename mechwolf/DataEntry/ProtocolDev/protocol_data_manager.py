...
class ProtocolDataManager:
    ...
    def save_protocol_config(self, protocol_config):
        ...
        new_entry = {
            'name': protocol_config.get('name', ''),  # Default to empty string if not provided
            'description': protocol_config.get('description', ''),  # Default to empty string if not provided
        ...
        try:
            ...
            print(f"Protocol '{new_entry['name']}' saved with {len(new_entry['pump_entries'])} entries")
        except Exception as e:
