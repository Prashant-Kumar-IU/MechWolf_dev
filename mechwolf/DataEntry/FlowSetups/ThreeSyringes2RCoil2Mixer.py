...
class ComponentApp:
    ...
    def create_widgets(self):
        ...
        # Add Vessel 3 inputs
        self.vessel3_name_input = widgets.Text(placeholder="Enter Vessel 3 name", layout=widgets.Layout(width='50%'))
        self.vessel3_desc_input = widgets.Text(placeholder="Enter Vessel 3 description", layout=widgets.Layout(width='50%'))
        ...
        # Update Coil inputs to include B and Y
        self.coil_b_input = widgets.Text(placeholder="Coil B length", layout=widgets.Layout(width='50%'))
        self.coil_y_input = widgets.Text(placeholder="Coil Y length", layout=widgets.Layout(width='50%'))
        ...
        if self.existing_config:
            ...
            # Safely handle variable number of vessels
            vessels = self.existing_config['vessels']
            if len(vessels) >= 4:  # Ensure we have all vessels
                ...
                self.vessel3_name_input.value = vessel3['name']
                self.vessel3_desc_input.value = vessel3['description']
            ...
            # Handle mixer configuration
            self.using_mixer_checkbox.value = self.existing_config.get('using_mixer', False)
            if self.using_mixer_checkbox.value and 'mixer' in self.existing_config['tubes']:
            ...
            # Safely load coil configurations
            coils = self.existing_config.get('coils', [])
            if len(coils) > 0:
                self.coil_a_input.value = coils[0]['length']
            if len(coils) > 1:
                self.coil_x_input.value = coils[1]['length']
            if len(coils) > 2:
                self.coil_b_input.value = coils[2]['length']
            if len(coils) > 3:
                self.coil_y_input.value = coils[3]['length']
        ...
        self.widget_container = widgets.VBox([
            ...
            widgets.Label("Vessel 3 Details:"),
            self.vessel3_name_input,
            self.vessel3_desc_input,
    ...
    def create_setup(self, b):
        ...
        try:
            self.data.update({
                ...
                'vessel3_name': self.vessel3_name_input.value or "",
                'vessel3_desc': self.vessel3_desc_input.value or "",
                ...
                'coil_b_raw': self.coil_b_input.value or "",
                'coil_y_raw': self.coil_y_input.value or "",
        ...
        required_fields = {
            ...
            'Vessel 3 Name': self.data['vessel3_name'],
            ...
            'Coil B Length': self.data['coil_b_raw'],
            'Coil Y Length': self.data['coil_y_raw']
        }
        ...
        self.data['coil_b_length'] = parse_numeric_foot(self.data['coil_b_raw'])
        self.data['coil_y_length'] = parse_numeric_foot(self.data['coil_y_raw'])
        if None in [self.data['coil_a_length'], self.data['coil_x_length'], self.data['coil_b_length'], self.data['coil_y_length']]:
            print("Error: Please enter valid numeric values for all coil lengths.")
            return
        ...
        required_fields = {
            ...
            'vessel3_name': self.data['vessel3_name'],
            'product_vessel_name': self.data['product_vessel_name'],
            'reaction_tube_ID': self.data['reaction_tube_ID'],
            'coil_lengths': [self.data['coil_a_length'], self.data['coil_x_length'], self.data['coil_b_length'], self.data['coil_y_length']]
        }
...
class ApparatusCreator:
    ...
    def create_apparatus(self):
        ...
        coil_b = reaction_tube(config['coils'][2]['length'])
        coil_y = reaction_tube(config['coils'][3]['length'])
