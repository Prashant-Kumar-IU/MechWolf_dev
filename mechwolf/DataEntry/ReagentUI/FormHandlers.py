...
class ReagentFormHandler:
    ...
    def create_form_field(
        widget: widgets.Widget, tooltip_text: str, error_style: bool = False
    ) -> widgets.VBox:
        """
    ...
    def create_reagent_form(
        ...
        form_fields = [form_title, error_area]
        ...
        form_fields.extend(
            [
                ...
                ReagentFormHandler.create_form_field(
                    smiles_input,
                    "Example: O=C[C@@H]([C@H]([C@@H]([C@@H](CO)O)O)O)O (glucose)",
                ),
                ReagentFormHandler.create_form_field(
        ...
        if reagent_type == "liquid":
            ...
            density_tooltip = (
                "Required for liquids: Please update this value!"
                if warning_message
                else "Required for liquids: Must be > 0"
            )
            form_fields.append(
                ReagentFormHandler.create_form_field(
        ...
        def update_structure(change=None):
            ...
            with structure_area:
                ...
                if smiles_input.value:
                    vis = StructureVisualizer.get_structure_image(
                        smiles_input.value, size=(200, 200)
                    )
                    if vis:
                        display(vis)
                    else:
        ...
        form_fields.append(
            widgets.VBox(
                [widgets.HTML("<h4>Structure Preview</h4>"), structure_area],
            ...
            def validate_and_save(b):
                ...
                try:
                    ...
                    if success:
                        ...
                        if (
                            not reagent
                        ):  # Only clear if this is a new entry (not editing)
                            name_input.value = ""
                            inchi_input.value = ""
                            smiles_input.value = ""
...
class FinalDetailsFormHandler:
    ...
    @staticmethod
    def create_final_details_form(
        data: Dict[str, Any], on_submit: Callable
    ) -> widgets.Widget:
        """
        ...
        def update_volume(*args):
            try:
                ...
                volume_display.value = (
                    f"<p><b>Volume needed:</b> {volume_solution:.4f} mL</p>"
                )
            except Exception as e:
                volume_display.value = (
                    f"<p><b>Volume needed:</b> Error in calculation: {str(e)}</p>"
                )
