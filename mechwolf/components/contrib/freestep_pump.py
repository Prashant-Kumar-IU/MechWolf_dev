...
class FreeStepPump(mw.Pump):
    ...
    def __init__(
        self, serial_port, mcu_id, motor_id, syringe_volume, syringe_diameter, name=None
    ):
    ...
    def __enter__(self):
        ...
        self._mcu_profile = next(
            (mcu for mcu in mcus if mcu.get("uniqueID") == self.mcu_id), None
        )
        self._motor_profile = next(
            (motor for motor in motors if motor.get("uniqueID") == self.motor_id), None
        )
        ...
        self._controller.stop_command(
            self.serial_port, self._mcu_profile, self._motor_profile
        )
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        try:
            ...
            if self.serial_port in _port_users:
                ...
                if (
                    not _port_users[self.serial_port]
                    and self.serial_port in _shared_controllers
                ):
    ...
    async def _update(self):
        ...
        if (
            not self._port_initialized
            or not self._mcu_profile
            or not self._motor_profile
        ):
        ...
        if rate_mlmin != self._last_rate:
            print(
                f"Rate change detected for {self.name}: {self._last_rate} -> {rate_mlmin} mL/min"
            )
            ...
            if abs_rate <= 0.000001:  # Near-zero rate threshold for stopping
                ...
                if (
                    ...
                    and calibrated_diameter != current_diameter
                ):
                    ...
                    print(
                        f"Adjusting for different syringe diameter: {current_diameter}mm vs calibrated {calibrated_diameter}mm"
                    )
                ...
                if success:
                    print(
                        f"Pump {self.name} now running at {abs_rate} mL/min ({direction})"
                    )
                else:
