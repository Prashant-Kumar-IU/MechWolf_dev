...
class JupyterCalibrationTool:
    ...
    def create_widgets(self):
        ...
        self.status_output = widgets.Output(
            layout=widgets.Layout(width="100%", border="1px solid #ddd")
        )
        ...
        self.port_dropdown = widgets.Dropdown(
            description="Port:", options=[], layout=widgets.Layout(width="50%")
        )
        self.refresh_ports_button = widgets.Button(
            description="Refresh Ports", button_style="info", icon="refresh"
        )
        self.connect_button = widgets.Button(
            description="Connect", button_style="success", icon="plug"
        )
        ...
        self.mcu_dropdown = widgets.Dropdown(
            description="MCU Profile:", options=[], layout=widgets.Layout(width="50%")
        )
        self.refresh_mcus_button = widgets.Button(
            description="Refresh MCUs", button_style="info", icon="refresh"
        )
        ...
        self.add_mcu_button = widgets.Button(
            description="Add MCU Profile", button_style="success", icon="plus"
        )
        ...
        self.motor_dropdown = widgets.Dropdown(
            description="Motor Profile:", options=[], layout=widgets.Layout(width="50%")
        )
        self.refresh_motors_button = widgets.Button(
            description="Refresh Motors", button_style="info", icon="refresh"
        )
        ...
        self.add_motor_button = widgets.Button(
            description="Add Motor Profile", button_style="success", icon="plus"
        )
        ...
        self.calibration_mcu_info = widgets.HTML(
            value="<p>No motor selected</p>", layout=widgets.Layout(width="100%")
        )

        # Pin configuration widgets
        self.step_pin_input = widgets.IntText(
            description="Step Pin:", value=0, layout=widgets.Layout(width="40%")
        )
        self.dir_pin_input = widgets.IntText(
            description="Dir Pin:", value=0, layout=widgets.Layout(width="40%")
        )
        self.configure_pins_button = widgets.Button(
            description="Configure Pins", button_style="primary", icon="wrench"
        )
        ...
        self.first_trial_duration_input = widgets.FloatText(
            description="Duration (s):", value=10.0, layout=widgets.Layout(width="40%")
        )
        self.first_trial_measurement_input = widgets.FloatText(
            description="Volume (mL):", value=0.0, layout=widgets.Layout(width="40%")
        )
        self.run_first_trial_button = widgets.Button(
            description="Run First Trial", button_style="warning", icon="play"
        )
        ...
        self.run_second_trial_button = widgets.Button(
            description="Run Second Trial", button_style="warning", icon="play"
        )
        ...
        self.test_direction = widgets.RadioButtons(
            options=["forward", "backward"], description="Direction:", value="forward"
        )
        self.test_duration_input = widgets.FloatText(
            description="Duration (s):", value=5.0, layout=widgets.Layout(width="40%")
        )
        ...
        self.run_test_button = widgets.Button(
            description="Run Test", button_style="primary", icon="play"
        )
        self.stop_test_button = widgets.Button(
            description="Stop Motor", button_style="danger", icon="stop"
        )

        # MCU profile editing widgets
        self.edit_mcu_name = widgets.Text(
            description="Edit Name:", layout=widgets.Layout(width="40%"), disabled=True
        )
        ...
        self.delete_mcu_button = widgets.Button(
            description="Delete MCU", button_style="danger", icon="trash", disabled=True
        )
        ...
        self.edit_motor_name = widgets.Text(
            description="Edit Name:", layout=widgets.Layout(width="60%"), disabled=True
        )
        ...
        self.associate_motor_dropdown = widgets.Dropdown(
            description="Motor:", options=[], layout=widgets.Layout(width="50%")
        )
        self.associate_mcu_dropdown = widgets.Dropdown(
            description="MCU:", options=[], layout=widgets.Layout(width="50%")
        )
        ...
        self.associate_button = widgets.Button(
            description="Associate Motor with MCU", button_style="primary", icon="link"
        )
        ...
        self.disconnect_button.on_click(
        ...
        self.calibration_motor_dropdown.observe(
            self.calibration_motor_changed, names="value"
        )
        self.test_mcu_dropdown.observe(self.update_test_selections, names="value")
        self.test_motor_dropdown.observe(
            self.test_motor_dropdown_changed, names="value"
        )
        ...
        self.test_use_calibrated_checkbox.observe(
            self.toggle_diameter_field, names="value"
        )
    ...
    def display_ui(self):
        ...
        from mechwolf.components.contrib.calibration_visualizations import (
        ...
        mcu_profiles_tab = widgets.VBox(
            [
                ...
                widgets.HBox(
                    [self.new_mcu_name, self.add_mcu_button, self.refresh_mcus_button]
                ),
                widgets.HTML("<h3>Edit Selected MCU</h3>"),
                widgets.HBox(
                    [self.edit_mcu_name, self.save_mcu_button, self.delete_mcu_button]
                ),
        ...
        motor_profiles_tab = widgets.VBox(
            [
                ...
                widgets.HBox(
                    [
                        self.update_pins_button,
                        self.save_motor_button,
                        self.delete_motor_button,
                    ]
                ),
                # Association section for new associations
                self.associate_title,
                widgets.HTML(
                    "<p>Associate a motor with a new MCU and set its pins</p>"
                ),
                widgets.HBox(
                    [self.associate_motor_dropdown, self.associate_mcu_dropdown]
                ),
        ...
        calibration_tab = widgets.VBox(
            [
                ...
                widgets.HTML(
                    "<p>Please provide information about the syringe used for calibration:</p>"
                ),
                ...
                widgets.HBox(
                    [self.first_trial_freq_input, self.first_trial_duration_input]
                ),
                self.first_trial_measurement_input,
                self.run_first_trial_button,
                widgets.HTML("<h3>Second Calibration Trial</h3>"),
                widgets.HBox(
                    [self.second_trial_freq_input, self.second_trial_duration_input]
                ),
                    ...
                    [
                        ...
                        widgets.VBox(
                            [
                                ...
                                widgets.HBox(
                                    [self.run_test_button, self.stop_test_button]
                                ),
                            ],
                            layout=widgets.Layout(width="40%"),
                        ),
    ...
    def refresh_ports(self, _=None):
        ...
        # Update the dropdown
        self.port_dropdown.options = [("Select Port", None)] + [
            (p["path"], p["path"]) for p in ports
        ]
    ...
    def refresh_motors(self, _=None):
        ...
        options = [("Select Motor", None)] + [
            (motor["name"], motor) for motor in motors
        ]
    ...
    def add_motor(self, _=None):
        ...
        if unique_id:
            ...
            self.log(
                "To use this motor, please associate it with an MCU using the section below."
            )
        ...
        if self.selected_mcu and self.selected_motor:
            try:
                self.controller.stop_command(
                    self.selected_port, self.selected_mcu, self.selected_motor
                )
    ...
    def run_calibration_trial(self, trial_name):
        ...
        if not self.selected_mcu:
            self.log(
                "No MCU available for this motor. Configure the motor in an MCU first.",
                clear=True,
            )
            return
        ...
        self.log(
            f"Running {trial_name} trial at {freq} Hz for {duration} seconds...",
            clear=True,
        )
        ...
        if not success:
            self.log(
                "Failed to send command to the device. Check connections.", clear=True
            )
            return

        self.log(
            f"Sent command to {self.selected_port}: timed run at {freq}Hz for {duration}s",
            clear=False,
        )
        ...
        self.log(
            f"{trial_name} trial complete. Please enter the measured volume.",
            clear=True,
        )
            ...
            for i, mcu in enumerate(mcus):
                ...
                if "motors" in mcu and mcu["motors"]:
                    ...
                    for motor in mcu["motors"]:
                        print(
                            f"   - {motor.get('name')} (Step: {motor.get('step')}, Dir: {motor.get('dir')})"
                        )
                else:
            ...
            for i, motor in enumerate(motors):
                ...
                if "syringeInfo" in motor:
                    ...
                    if "innerDiameterMM" in si:  # Use the new key if available
                        ...
                        diameter_label = "dia"
                    print(
                        f"   Syringe: {si.get('brand')} {si.get('model')}, {si.get('volumeML')}mL, {si.get(diameter_key)}mm {diameter_label}"
                    )
                    print(
                        f"   Calibration Date: {si.get('calibrationDate', 'Unknown')}"
                    )
                ...
                for mcu in mcus:
                    for mcu_motor in mcu.get("motors", []):
                        if mcu_motor.get("uniqueID") == motor_id:
                            print(
                                f"   Used in MCU '{mcu.get('name')}' with Step: {mcu_motor.get('step')}, Dir: {mcu_motor.get('dir')}"
                            )
    ...
    def edit_motor(self, motor_id):
        ...
        if motor:
            ...
            if associated_mcus:
                self.edit_motor_mcu_dropdown.options = [
                    ("Select MCU", None)
                ] + associated_mcus
                self.edit_motor_mcu_dropdown.disabled = False
                self.edit_motor_mcu_dropdown.observe(
                    self.motor_mcu_association_changed, names="value"
                )
            else:
            ...
            if "syringeInfo" in motor:
                si = motor["syringeInfo"]
                self.log(
                    f"Calibrated with {si.get('brand')} {si.get('model')} syringe ({si.get('volumeML')}mL, {si.get('diameterMM')}mm dia)"
                )

            if not associated_mcus:
                self.log(
                    "This motor is not associated with any MCU. Use the Associate Motor with MCU section below."
                )
    ...
    def update_motor_pins(self, _):
        ...
        if motor_updated:
            ...
            self.log(
                f"Updated pin assignments in MCU '{selected_mcu.get('name')}': step={step_pin}, dir={dir_pin}"
            )
    ...
    def save_motor_changes(self, _):
        ...
        motors = self.controller.get_motors()
        motor = next(
            (m for m in motors if m.get("uniqueID") == self.current_motor_id), None
        )
            ...
            for mcu in mcus:
                if "motors" in mcu:
                    ...
                    mcu["motors"] = [
                        m
                        for m in mcu["motors"]
                        if m.get("uniqueID") != self.current_motor_id
                    ]
    ...
    def configure_pins(self, _):
        ...
        self.log(
            f"Configuring pins for {selected_motor['name']} in {selected_mcu['name']}...",
            clear=True,
        )
    ...
    def calculate_calibration(self, _):
        ...
        if syringe_volume <= 0 or syringe_diameter <= 0:
            self.log(
                "Please enter valid syringe volume and diameter values", clear=True
            )
            return
        ...
        self.first_trial = {
            ...
            "ups": (first_measurement / self.first_trial_duration_input.value)
            * 60,  # Convert to mL/min
        }
        ...
        self.second_trial = {
            ...
            "ups": (second_measurement / self.second_trial_duration_input.value)
            * 60,  # Convert to mL/min
        }
        ...
        ups_slope = (self.second_trial["ups"] - self.first_trial["ups"]) / (
            self.second_trial["freq"] - self.first_trial["freq"]
        )
    ...
    def run_test(self, _):
        ...
        custom_diameter = (
            self.test_syringe_diameter.value
            if not self.test_use_calibrated_checkbox.value
            else 0.0
        )
        ...
        self.log(
            f"Running motor at {ups} mL/min {direction} for {duration} seconds...",
            clear=True,
        )
        ...
        freq = self.controller.command_processor.convert_ups_to_freq(
            selected_motor, ups
        )
        if freq is None:
            return
        ...
        self.log(
            f"Using calculated frequency: {freq:.2f}Hz (rounded to {freq_int}Hz for stepper motor)",
            clear=False,
        )
        ...
        if custom_diameter > 0.0:
            command["syringeDiameter"] = float(
                custom_diameter
            )  # Ensure proper float formatting
            self.log(
                f"Using custom syringe inner diameter: {custom_diameter}mm", clear=False
            )
        ...
        # Add calibrated syringe diameter if available and not using custom
            ...
            if "innerDiameterMM" in si:
                ...
                self.log(
                    f"Using calibrated syringe inner diameter: {si.get('innerDiameterMM')}mm",
                    clear=False,
                )
            elif "diameterMM" in si:
                command["syringeDiameter"] = si.get("diameterMM")
                self.log(
                    f"Using calibrated syringe diameter: {si.get('diameterMM')}mm",
                    clear=False,
                )

        # Send the command directly
        success = self.controller.serial_manager.send_command(
            self.selected_port, command
        )
        ...
        if success:
            ...
            self.controller.stop_command(
                self.selected_port, selected_mcu, selected_motor
            )
            time.sleep(0.5)  # Give time for stop command to process
    ...
    def stop_motor(self, _):
        ...
        success = self.controller.stop_command(
            self.selected_port, selected_mcu, selected_motor
        )
    ...
    def update_test_selections(self, change):
        ...
        mcu_motor_ids = [
            motor.get("uniqueID") for motor in selected_mcu.get("motors", [])
        ]
        calibrated_motors = [
            motor
            for motor in motors
            if motor.get("uniqueID") in mcu_motor_ids and motor.get("calibrated")
        ]

        # Update the test motor dropdown
        self.test_motor_dropdown.options = [("Select Motor", None)] + [
            (motor["name"], motor) for motor in calibrated_motors
        ]
    ...
    def update_calibration_details(self):
        ...
        with self.test_calibration_details:
            ...
            if "syringeInfo" in selected_motor:
                ...
                print(
                    f"  - Volume: {si.get('volumeML')}mL, {diameter_label}: {calibrated_diameter}mm"
                )
                print(f"  - Calibration Date: {cal_date}")

                # Calculate base frequency
                freq = self.controller.command_processor.convert_ups_to_freq(
                    selected_motor, flow_rate
                )
                ...
                if custom_diameter > 0.0:
                    ...
                    print(
                        f"  - Frequency Adjustment: {freq_int}Hz → {int(round(adjusted_freq))}Hz"
                    )

                    # Explain what this means
                    if ratio > 1:
                        print(
                            f"  - Effect: Motor will run FASTER to maintain flow rate"
                        )
                        print(
                            f"    (Custom syringe is narrower than calibrated syringe)"
                        )
                    else:
                        print(
                            f"  - Effect: Motor will run SLOWER to maintain flow rate"
                        )
    ...
    def calibration_motor_changed(self, change):
        ...
        if not motor_config:
            self.calibration_mcu_info.value = (
                "<p style='color: red;'>Motor configuration not found.</p>"
            )
            self.run_first_trial_button.disabled = True
            self.run_second_trial_button.disabled = True
            return
    ...
    def update_association_dropdowns(self):
        ...
        motors = self.controller.get_motors()
        self.associate_motor_dropdown.options = [("Select Motor", None)] + [
            (motor["name"], motor) for motor in motors
        ]

        # Update MCU dropdown
        mcus = self.controller.get_mcus()
        self.associate_mcu_dropdown.options = [("Select MCU", None)] + [
            (mcu["name"], mcu) for mcu in mcus
        ]
    ...
    def associate_motor_with_mcu(self, _):
        ...
        self.log(
            f"Associating motor '{selected_motor['name']}' with MCU '{selected_mcu['name']}'...",
            clear=True,
        )
        ...
        if motor_in_mcu:
            ...
            motor_in_mcu["dir"] = dir_pin
            self.log(
                f"Updated existing motor association with step={step_pin}, dir={dir_pin}"
            )
        else:
            if "motors" not in selected_mcu:
                selected_mcu["motors"] = []
            ...
            self.log(
                f"Created new motor association with step={step_pin}, dir={dir_pin}"
            )

        # Save the updated MCU profile
        self.controller.profile_manager.update_mcu(selected_mcu)
        self.log(
            f"Motor '{selected_motor['name']}' successfully associated with MCU '{selected_mcu['name']}'"
        )
