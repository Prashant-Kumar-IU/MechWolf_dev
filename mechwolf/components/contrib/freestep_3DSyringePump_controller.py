...
class SerialManager:
    ...
    def list_ports(self):
        ...
        return [
            {"path": port.device, "description": port.description}
    ...
    def open_port(self, port_name, baudrate=9600):
        ...
        try:
            ...
            thread = threading.Thread(
                target=self._read_responses, args=(port_name,), daemon=True
            )
            self.read_threads[port_name] = thread
            thread.start()
    ...
    def send_command(self, port_name, command_data):
        ...
        try:
            ...
            if "syringeDiameter" in command_data and "freq" in command_data:
                ...
                reference_diameter = command_data.get(
                    "referenceDiameter", 8.0
                )  # Default 8mm diameter
                ...
                area_ratio = (
                    reference_area / cross_sectional_area
                ...
                print(
                    f"Calibrated for syringe diameter: {diameter}mm (ratio: {area_ratio:.4f})"
                )
    ...
    def send_formatted_command(
        self, port_name, type, step_pin, dir_pin, freq, time_value, time_unit, direction
    ):
...
class ProfileManager:
    ...
    def add_mcu(self, name="New MCU"):
        ...
        mcus.append({"uniqueID": unique_id, "name": name, "motors": []})
        self.save_mcus(mcus)
        return unique_id
    ...
    def add_motor(self, name="New Motor"):
        ...
        unique_id = "".join(random.choices(string.ascii_lowercase + string.digits, k=9))
        motors.append({"uniqueID": unique_id, "name": name, "calibrated": False})
...
class CommandProcessor:
    ...
    def run_timed(
        self, com_port, direction, freq, step_pin, dir_pin, time_value, time_unit
    ):
    ...
    def stop_motor(self, com_port, step_pin, dir_pin):
        """Stops a motor"""
        command = {"type": "stop", "stepPin": step_pin, "dirPin": dir_pin}
        return self.serial_manager.send_command(com_port, command)
...
class FreeStepController:
    ...
    def __init__(self):
        ...
        self.command_processor = CommandProcessor(
            self.serial_manager, self.profile_manager
        )
    ...
    def run_basic_command(
        self, port, motor_profile, mcu_profile, ups, direction, diameter_ratio=1.0
    ):
        ...
        # Send the command
        return self.command_processor.run_basic(
            port, direction, freq, step_pin, dir_pin
        )

    def run_timed_command(
        self, port, motor_profile, mcu_profile, ups, time_value, time_unit, direction
    ):
        ...
        if "syringeInfo" in motor_profile:
            if "innerDiameterMM" in motor_profile["syringeInfo"]:
                command["syringeDiameter"] = motor_profile["syringeInfo"][
                    "innerDiameterMM"
                ]
                print(
                    f"Using syringe inner diameter: {motor_profile['syringeInfo']['innerDiameterMM']}mm"
                )
            elif "diameterMM" in motor_profile["syringeInfo"]:
                command["syringeDiameter"] = motor_profile["syringeInfo"]["diameterMM"]
                print(
                    f"Using syringe diameter: {motor_profile['syringeInfo']['diameterMM']}mm"
                )
    ...
    try:
        ...
        while True:
            ...
            if cmd == "help":
                ...
                print(
                    "  run <port> <mcu_id> <motor_id> <ups> <direction> - Run a motor"
                )
                print(
                    "  run_timed <port> <mcu_id> <motor_id> <ups> <time> <unit> <direction> - Run a motor for a time"
                )
                ...
                for motor in motors:
                    ...
                    print(
                        f"  ID: {motor.get('uniqueID')}, Name: {motor.get('name')}, Calibrated: {calibrated}"
                    )
                ...
                if len(parts) != 6:
                    ...
                    mcu_profile = next(
                        (mcu for mcu in mcus if mcu.get("uniqueID") == mcu_id), None
                    )
                    motor_profile = next(
                        (
                            motor
                    ...
                    if mcu_profile and motor_profile:
                        controller.run_basic_command(
                            port, motor_profile, mcu_profile, ups, direction
                        )
                    else:
                ...
                if len(parts) != 8:
                    print(
                        "Usage: run_timed <port> <mcu_id> <motor_id> <ups> <time> <unit> <direction>"
                    )
                else:
                    port = parts[1]
                    mcu_id = parts[2]
