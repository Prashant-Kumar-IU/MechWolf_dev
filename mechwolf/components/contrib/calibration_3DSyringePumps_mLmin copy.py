...
# Add Graphviz imports for network and workflow diagrams
import graphviz
...
class JupyterCalibrationTool:
    ...
    def create_overview_widgets(self):
        """Create widgets for the overview tab showing workflow and connections"""
        # Create output areas for the diagrams
        self.overview_workflow_output = widgets.Output(
            layout=widgets.Layout(
                border='1px solid #ddd',
                padding='10px',
                width='100%',
        ...
        self.overview_connections_output = widgets.Output(
            layout=widgets.Layout(
                border='1px solid #ddd',
                padding='10px',
                width='100%',
        ...
        # Create refresh button for connections diagram
        self.overview_refresh_button = widgets.Button(
        ...
        # Add button handler
        self.overview_refresh_button.on_click(self.refresh_overview_diagrams)

    def create_visualization_widgets(self):
        """Create widgets for the visualization tab"""
        # Motor selection for visualization
        self.visualization_motor_dropdown = widgets.Dropdown(
            description='Motor:',
            options=[],
            layout=widgets.Layout(width='60%')
        )
        
        self.visualization_refresh_button = widgets.Button(
            description='Refresh Motors',
            button_style='info',
            icon='refresh'
        )
        
        # Plot type selection - Remove Graphviz options that were moved to overview tab
        self.visualization_plot_type = widgets.Dropdown(
            description='Plot:',
            options=[
                ('Flow Rate vs Frequency', 'flow_freq'),
                ('Calibration Points', 'cal_points'),
                ('Operating Range', 'op_range')
            ],
            value='flow_freq',
            layout=widgets.Layout(width='60%')
        )
        ...
        self.visualization_output = widgets.Output(
            layout=widgets.Layout(
                ...
                padding='10px',
                width='100%',
                height='500px',  # Taller to fit plots
                overflow='auto'
            )
        )
        
        # Add a description label
        self.visualization_description = widgets.HTML(
            value="<p>Select a calibrated motor to visualize its calibration data.</p>"
        )
    
    def display_ui(self):
        """Display the main UI with tabs"""
        # Overview & Workflow tab
        overview_tab = widgets.VBox([
            widgets.HTML("<h3>System Overview & Calibration Workflow</h3>"),
            widgets.HTML("<p>This overview shows the overall system connections and calibration workflow.</p>"),
            
            # Workflow section
            widgets.HTML("<h3>Calibration Workflow</h3>"),
            widgets.HTML("<p>Follow this workflow to calibrate a motor:</p>"),
            self.overview_workflow_output,
            
            # Connections section
            widgets.HTML("<h3>MCU-Motor Connections</h3>"),
            widgets.HTML("<p>Current connections between MCUs and motors:</p>"),
            widgets.HBox([self.overview_refresh_button]),
            self.overview_connections_output
        ])
        ...
        # Visualization tab (only motor-specific plots now)
        visualization_tab = widgets.VBox([
            widgets.HTML("<h3>Visualize Motor Calibration</h3>"),
            self.visualization_description,
            widgets.HBox([self.visualization_motor_dropdown, self.visualization_refresh_button]),
            self.visualization_plot_type,
            self.visualization_output
        ])
        
        # Set up the tabs - Add overview tab as first tab
        self.tabs.children = [
            overview_tab,
            connection_tab,
            mcu_profiles_tab,
        ...
        self.tabs.set_title(0, 'Overview & Workflow')
        ...
        # Initially populate the visualization motors dropdown
        self.refresh_visualization_motors()
        
        # Initialize diagrams in overview tab
        self.refresh_overview_diagrams()
    ...
    def calculate_calibration(self, _):
        ...
        # Save trial data in the motor profile for visualization
        self.selected_motor["calibrationData"] = {
            "firstTrial": self.first_trial,
            "secondTrial": self.second_trial
        }
    ...
    def refresh_visualization_motors(self, _=None):
        """Refresh the motor dropdown in the visualization tab"""
        self.log("Loading calibrated motors for visualization...", clear=True)
        
        # Get all motors and filter for calibrated ones
        all_motors = self.controller.get_motors()
        calibrated_motors = [motor for motor in all_motors if motor.get('calibrated', False)]
        
        # Update the dropdown
        self.visualization_motor_dropdown.options = [('Select Motor', None)] + [(motor['name'], motor) for motor in calibrated_motors]
        self.visualization_motor_dropdown.value = None
        
        self.log(f"Found {len(calibrated_motors)} calibrated motors")
    
    def refresh_overview_diagrams(self, _=None):
        """Refresh both diagrams in the overview tab"""
        # Update the workflow diagram
        with self.overview_workflow_output:
            clear_output(wait=True)
            self.create_calibration_workflow_diagram(self.overview_workflow_output)
        
        # Update the connections diagram
        with self.overview_connections_output:
            clear_output(wait=True)
            self.create_mcu_motor_diagram(self.overview_connections_output)

    def create_mcu_motor_diagram(self, output_widget=None):
        """Create a Graphviz diagram showing MCU-Motor relationships"""
        if output_widget is None:
            output_widget = self.visualization_output
            
        try:
            # Get all MCUs and motors
            mcus = self.controller.get_mcus()
            motors = self.controller.get_motors()
            ...
            # Create a subgraph for MCUs
            with dot.subgraph(name='cluster_mcus') as c:
                c.attr(label='MCU Controllers', style='filled', color='lightgrey')
                
                for mcu in mcus:
                    mcu_id = mcu.get('uniqueID')
                    mcu_name = mcu.get('name')
                    c.node(f'mcu_{mcu_id}', f"{mcu_name}\nID: {mcu_id[:8]}...")
                
            # Create a subgraph for Motors
            with dot.subgraph(name='cluster_motors') as c:
                c.attr(label='Motors', style='filled', color='lightgrey')
                
                for motor in motors:
                    motor_id = motor.get('uniqueID')
                    motor_name = motor.get('name')
                    calibrated = "✓" if motor.get('calibrated') else "✗"
                    
                    # Different color for calibrated vs uncalibrated
                    node_color = '#90EE90' if motor.get('calibrated') else '#FFB6C1'  # Light green vs light red
                    
                    label = f"{motor_name}\nID: {motor_id[:8]}...\nCalibrated: {calibrated}"
                    
                    # Add syringe info for calibrated motors
                    if motor.get('calibrated') and 'syringeInfo' in motor:
                        si = motor["syringeInfo"]
                        label += f"\nSyringe: {si.get('brand')} {si.get('model')}"
                        if "innerDiameterMM" in si:
                            label += f"\n{si.get('volumeML')}mL, {si.get('innerDiameterMM')}mm inner dia"
                        elif "diameterMM" in si:
                            label += f"\n{si.get('volumeML')}mL, {si.get('diameterMM')}mm dia"
                    
                    c.node(f'motor_{motor_id}', label, color=node_color)
            
            # Add edges to connect MCUs to motors with pin information
            for mcu in mcus:
                mcu_id = mcu.get('uniqueID')
                
                for motor_config in mcu.get('motors', []):
                    motor_id = motor_config.get('uniqueID')
                    step_pin = motor_config.get('step')
                    dir_pin = motor_config.get('dir')
                    
                    # Add edge with pin info
                    dot.edge(f'mcu_{mcu_id}', f'motor_{motor_id}', 
                            label=f"Step: {step_pin}\nDir: {dir_pin}",
                            color='blue')
            
            # Render the diagram
            temp_file = dot.render(filename='mcu_motor_diagram', directory='/tmp', cleanup=True)
            ipydisplay(Image(filename=temp_file))
            
            print("Diagram shows the connections between MCUs (controllers) and Motors.")
            print("Green motor boxes indicate calibrated motors, pink indicates uncalibrated.")
            
        except Exception as e:
            print(f"Error creating MCU-Motor diagram: {str(e)}")
            import traceback
            traceback.print_exc()

    def create_calibration_workflow_diagram(self, output_widget=None):
        """Create a Graphviz diagram showing the calibration workflow"""
        if output_widget is None:
            output_widget = self.visualization_output
            
        try:
            # Create a new Digraph object for the workflow
            dot = graphviz.Digraph(comment='Calibration Workflow', 
                                  format='png', 
                                  engine='dot')
            
            dot.attr(rankdir='TB', size='8,10', 
                    fontname='Arial', fontsize='14', ratio='fill',
                    bgcolor='white')
            
            # Define node styles for different steps
            setup_style = {'shape': 'box', 'style': 'filled', 'color': 'lightskyblue', 'fontname': 'Arial'}
            run_style = {'shape': 'box', 'style': 'filled', 'color': 'lightgreen', 'fontname': 'Arial'}
            decision_style = {'shape': 'diamond', 'style': 'filled', 'color': 'lightyellow', 'fontname': 'Arial'}
            result_style = {'shape': 'box', 'style': 'filled', 'color': 'lightpink', 'fontname': 'Arial'}
            
            # Setup stage
            dot.node('connect', 'Connect to Controller', **setup_style)
            dot.node('select_motor', 'Select Motor Profile', **setup_style)
            dot.node('set_pins', 'Configure MCU Pins', **setup_style)
            dot.node('syringe_info', 'Enter Syringe Information', **setup_style)
            
            # First trial
            dot.node('first_trial', 'Run First Trial\n(Low Frequency)', **run_style)
            dot.node('measure_first', 'Measure Dispensed Volume', **run_style)
            
            # Second trial
            dot.node('second_trial', 'Run Second Trial\n(High Frequency)', **run_style)
            dot.node('measure_second', 'Measure Dispensed Volume', **run_style)
            
            # Calculation
            dot.node('calculate', 'Calculate Calibration Parameters', **result_style)
            dot.node('save', 'Save Calibration Profile', **result_style)
            dot.node('visualize', 'Visualize Results', **result_style)
            dot.node('test', 'Test Flow Rates', **result_style)
            
            # Connect nodes - setup
            dot.edge('connect', 'select_motor')
            dot.edge('select_motor', 'set_pins')
            dot.edge('set_pins', 'syringe_info')
            
            # First trial
            dot.edge('syringe_info', 'first_trial')
            dot.edge('first_trial', 'measure_first')
            
            # Second trial
            dot.edge('measure_first', 'second_trial')
            dot.edge('second_trial', 'measure_second')
            
            # Calculation and results
            dot.edge('measure_second', 'calculate')
            dot.edge('calculate', 'save')
            dot.edge('save', 'visualize')
            dot.edge('save', 'test')
            
            # Render the diagram
            temp_file = dot.render(filename='calibration_workflow', directory='/tmp', cleanup=True)
            ipydisplay(Image(filename=temp_file))
            
            print("This diagram shows the recommended workflow for calibrating a motor.")
            print("Blue: Setup steps | Green: Run steps | Pink: Results")
            
        except Exception as e:
            print(f"Error creating workflow diagram: {str(e)}")
            import traceback
            traceback.print_exc()

    def update_visualization(self, _=None):
        """Update the visualization plot based on the selected motor and plot type"""
        selected_motor = self.visualization_motor_dropdown.value
        plot_type = self.visualization_plot_type.value
        
        with self.visualization_output:
            clear_output(wait=True)
            
            # For motor-specific visualizations, we need a selected motor
            if not selected_motor:
                print("Please select a calibrated motor")
                return
            
            if not selected_motor.get('calibrated'):
                print("This motor is not calibrated")
                return
                
            try:
                if plot_type in ['flow_freq', 'cal_points', 'op_range']:
                    # Existing matplotlib visualization code for motor-specific plots
                    ...
                    if 'calibrationData' in selected_motor:
                        cal_data = selected_motor['calibrationData']
                        if 'firstTrial' in cal_data:
                            first_trial = cal_data['firstTrial']
                        if 'secondTrial' in cal_data:
                            second_trial = cal_data['secondTrial']
                    
                    # For basic calculation without stored trial data
                    max_freq = 1000  # Hz, typical maximum for stepper motors
                    
                    # Generate x values for the plot (frequency in Hz)
                    x = np.linspace(0, max_freq, 100)
                    
                    # Calculate y values (flow rate in mL/min)
                    y = slope * x + intercept
                    
                    # Clip to minimum flow rate
                    y = np.maximum(y, min_ups)
                    
                    if plot_type == 'flow_freq':
                        # Plot flow rate vs frequency
                        ax.plot(x, y, 'b-', linewidth=2, label='Flow Rate Response')
                        ax.set_xlabel('Frequency (Hz)')
                        ax.set_ylabel('Flow Rate (mL/min)')
                        ax.set_title(f'Flow Rate vs Frequency for {selected_motor["name"]}')
                        
                        # Add operating range guides
                        ax.axhline(y=min_ups, color='r', linestyle='--', alpha=0.7, label=f'Min Flow Rate: {min_ups:.6f} mL/min')
                        ax.axhline(y=max_ups, color='g', linestyle='--', alpha=0.7, label=f'Max Flow Rate: {max_ups:.2f} mL/min')
                        
                        # Add equation to the plot
                        equation_text = f"Flow Rate = {slope:.6f} × Freq + {intercept:.6f}"
                        ax.text(0.05, 0.95, equation_text, transform=ax.transAxes, fontsize=10, 
                                verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
                        
                    elif plot_type == 'cal_points':
                        # Plot calibration points
                        ax.plot(x, y, 'b-', linewidth=2, alpha=0.5, label='Calibration Line')
                        
                        # Add calibration points if available
                        if first_trial and 'freq' in first_trial and 'ups' in first_trial:
                            ax.scatter([first_trial['freq']], [first_trial['ups']], color='r', s=100, label='First Trial')
                            
                        if second_trial and 'freq' in second_trial and 'ups' in second_trial:
                            ax.scatter([second_trial['freq']], [second_trial['ups']], color='g', s=100, label='Second Trial')
                        
                        # If we don't have stored trial data, make a best guess from slope/intercept
                        if not first_trial and not second_trial:
                            # Guess frequencies that might have been used for calibration
                            freq1 = 100  # Common first test point
                            freq2 = 500  # Common second test point
                            
                            ups1 = slope * freq1 + intercept
                            ups2 = slope * freq2 + intercept
                            
                            ax.scatter([freq1], [ups1], color='r', s=100, label='Estimated First Point')
                            ax.scatter([freq2], [ups2], color='g', s=100, label='Estimated Second Point')
                            
                            # Add note that these are estimated
                            ax.text(0.05, 0.05, "Note: These points are estimated from the\ncalibration equation, not actual trial data.", 
                                    transform=ax.transAxes, fontsize=9, color='red',
                                    verticalalignment='bottom', bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
                            
                        ax.set_xlabel('Frequency (Hz)')
                        ax.set_ylabel('Flow Rate (mL/min)')
                        ax.set_title(f'Calibration Points for {selected_motor["name"]}')
                        
                    elif plot_type == 'op_range':
                        # Create a bar chart showing the operating range
                        y_range = [min_ups, max_ups-min_ups]  # Min and difference to max
                        ax.bar(['Min', 'Max'], [min_ups, max_ups], color=['#ff9999', '#66b3ff'])
                        ax.set_ylabel('Flow Rate (mL/min)')
                        ax.set_title(f'Operating Flow Rate Range for {selected_motor["name"]}')
                        
                        # Add value labels
                        for i, v in enumerate([min_ups, max_ups]):
                            if i == 0:
                                ax.text(i, v/2, f"{v:.6f}", ha='center', fontweight='bold')
                            else:
                                ax.text(i, v/2, f"{v:.2f}", ha='center', fontweight='bold')
                        
                        # Add the range calculation
                        range_text = f"Range: {min_ups:.6f} to {max_ups:.2f} mL/min\nDynamic Range: {max_ups/min_ups:.1f}x"
                        ax.text(0.5, 0.05, range_text, transform=ax.transAxes, fontsize=11, ha='center',
                                verticalalignment='bottom', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
                    
                    # Add syringe info if available
                    if "syringeInfo" in selected_motor:
                        si = selected_motor["syringeInfo"]
                        if "innerDiameterMM" in si:
                            diameter_key = "innerDiameterMM"
                        ...
                        syringe_text = f"Calibrated with: {si.get('brand')} {si.get('model')}\n"
                        syringe_text += f"{si.get('volumeML')}mL, {si.get(diameter_key)}mm {diameter_label}"
                        if 'calibrationDate' in si:
                            syringe_text += f"\nCalibrated: {si.get('calibrationDate')}"
                            
                        # Place syringe info in upper right
                        ax.text(0.95, 0.95, syringe_text, transform=ax.transAxes, fontsize=9,
