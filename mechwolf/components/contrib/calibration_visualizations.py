...
class CalibrationVisualizer:
    ...
    def create_widgets(self):
        ...
        self.motor_dropdown = widgets.Dropdown(
            description="Select Motor:", options=[], layout=widgets.Layout(width="50%")
        )
        ...
        self.generate_plot_button = widgets.Button(
            description="Generate Plot", button_style="success", icon="chart-line"
        )
        ...
        self.diameter_comparison_title = widgets.HTML(
            "<h3>Syringe Diameter Comparison</h3>"
        )
        ...
        self.generate_diameter_plot_button = widgets.Button(
            description="Compare Diameters", button_style="success", icon="chart-line"
        )
        ...
        self.generate_diameter_plot_button.on_click(
            self.generate_diameter_comparison_plot
        )
    ...
    def refresh_motors(self, _=None):
        ...
        self.motor_dropdown.options = [("Select Motor", None)] + [
            (motor["name"], motor) for motor in calibrated_motors
        ]
        self.diameter_motor_dropdown.options = [("Select Motor", None)] + [
            (motor["name"], motor) for motor in calibrated_motors
        ]
        ...
        with self.diameter_plot_output:
            ...
            if not calibrated_motors:
                ...
                print(
                    "Select a calibrated motor, enter a new diameter, and click 'Compare Diameters'."
                )
    ...
    def generate_calibration_plot(self, _=None):
        ...
        with self.calibration_plot_output:
            ...
            ax.axhspan(
                min_ups, max_ups, alpha=0.1, color="blue", label="Valid Flow Rate Range"
            )
            ...
            ax.plot(
                ...
                "ro",
                markersize=8,
                label=f"Min: {min_ups:.6f} mL/min @ {min_freq:.1f} Hz",
            )
            ax.plot(
                [max_freq],
                [max_ups],
                "go",
                markersize=8,
                label=f"Max: {max_ups:.2f} mL/min @ {max_freq:.1f} Hz",
            )
            ...
            ax.set_title(
                f'Calibration Curve for {selected_motor.get("name")}{syringe_info}',
                fontsize=12,
            )
            ...
            plt.tight_layout(
                pad=3.0, h_pad=None, w_pad=None, rect=[0, 0.08, 1, 0.95]
    ...
    def generate_diameter_comparison_plot(self, _=None):
        ...
        if not original_diameter:
            with self.diameter_plot_output:
                clear_output()
                print(
                    "Error: Could not find original syringe diameter in motor calibration data."
                )
            return
        ...
        with self.diameter_plot_output:
            ...
            fig = plt.figure(
                figsize=(12, 7.5)
            )  # Reduced from 9 to 7.5 to leave space for analysis
            ax = fig.add_subplot(111)
            ...
            ax.set_title(
                f'Diameter Comparison for {selected_motor.get("name")}', fontsize=14
            )
            ax.set_xlabel("Frequency (Hz)", fontsize=12)
            ax.set_ylabel("Flow Rate (mL/min)", fontsize=12)
            ...
            # Apply tight layout with increased padding
            plt.tight_layout(
                pad=3.0, rect=[0, 0.08, 1, 0.95]
            )  # Added rect parameter to increase bottom margin
    ...
    def generate_mcu_motor_graph(self, _=None):
        ...
        with self.graph_output:
            ...
            for mcu in mcus:
                ...
                dot.node(
                    f"mcu_{mcu_id}", f"{mcu_name}{port_info}", fillcolor="lightblue"
                )
            ...
            try:
                ...
                print(
                    "Note: You might also need to install the Graphviz system package."
                )
    ...
    def create_tab_layout(self):
        ...
        calibration_plot_section = widgets.VBox(
            [
                ...
                widgets.HTML(
                    "<p>Visualize the calibration curve for a selected motor profile</p>"
                ),
                widgets.HBox(
                    [
                        self.motor_dropdown,
                        self.refresh_motors_button,
                        self.generate_plot_button,
                    ]
                ),
        ...
        diameter_comparison_section = widgets.VBox(
            [
                ...
                widgets.HBox(
                    [self.new_diameter_input, self.generate_diameter_plot_button]
                ),
        ...
        return widgets.VBox(
            [
                ...
                ),  # Increased margin from 20px to 30px
    ...
    def create_overview_tab(self):
        ...
        overview_tab = widgets.VBox(
            [
                ...
                widgets.HTML(
                    "<p>This tool helps you calibrate syringe pumps by associating motors with microcontrollers and performing calibration trials.</p>"
                ),
                widgets.HBox([flow_chart_section, mcu_motor_section]),
                self.graph_output,
            ]
        )
